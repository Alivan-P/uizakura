...
class LifecycleRegistry extends Lifecycle {
  ...
  LinkedHashMap<LifecycleObserver, ObserverWithState> _observerMap =
      LinkedHashMap();
  ...
  LifecycleState calculateTargetState(LifecycleObserver observer) {
    final entries = _observerMap.entries.toList();
    MapEntry<LifecycleObserver, ObserverWithState>? previous;
    ...
    LifecycleState? siblingState =
        previous != null ? previous!.value.state : null;
  ...
  void forwardPass(LifecycleOwner lifecycleOwner) {
    Iterator<MapEntry<LifecycleObserver, ObserverWithState>> ascendingIterator =
        _observerMap.entries.toList().iterator;
    while (ascendingIterator.moveNext() && !_mNewEventOccurred) {
      MapEntry<LifecycleObserver, ObserverWithState> entry =
  ...
  void backwardPass(LifecycleOwner lifecycleOwner) {
    Iterator<MapEntry<LifecycleObserver, ObserverWithState>>
        descendingIterator = _observerMap.entries.toList().reversed.iterator;
  ...
  void _sync() {
    ...
    while (!isSynced()) {
      ...
      MapEntry<LifecycleObserver, ObserverWithState>? newest =
...
class ObserverWithState {
  ...
  factory ObserverWithState.observer(LifecycleObserver observer,
...
class _LifecycleEventObserverAdapter extends LifecycleEventObserver {
  ...
  void onStateChanged(LifecycleOwner source, LifecycleEvent event) {
    ...
    if (observer is FullLifecycleObserver) {
      fullLifecycleObserver = observer as FullLifecycleObserver?;
    } else if (observer is LifecycleEventObserver) {
      lifecycleEventObserver = observer as LifecycleEventObserver?;
    }
    lifecycleEventObserver?.onStateChanged(source, event);
    switch (event) {
      case LifecycleEvent.onCreate:
        fullLifecycleObserver?.onCreate?.call(source);
        break;
      case LifecycleEvent.onVisible:
        fullLifecycleObserver?.onVisible?.call(source);
        break;
      case LifecycleEvent.onInvisible:
        fullLifecycleObserver?.onInvisible?.call(source);
        break;
      case LifecycleEvent.onDispose:
        fullLifecycleObserver?.onDispose?.call(source);
        break;
